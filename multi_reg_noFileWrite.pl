...
open(RCODE,">>rcode_v2") || die "$!\n";
print RCODE "outfile <- sprintf(\"Reg_Coefs_meanNorm\")\n";
print RCODE "cat(c(\"Pos\"";
for($i=1;$i<$#pres+1;$i++){
	print RCODE ",\"val.$pres[$i]\",\"pval.$pres[$i]\"";
}
print RCODE ",\"rsq\",\"F-statistic_P-value\",\"\\n\"),file=outfile,append = T,sep=\"\\t\")\n";
...
while(chomp($line=<FILE>)){
	...
	for($i=0;$i<=$#pres;$i++){
		$j=$i+2;
		print RCODE "dat[,$j] <- dat[,$j]/mean(dat[,$j])\n";
	}
	...
	for($i=1;$i<=$#pres;$i++){
		print RCODE "val.$pres[$i] <- summary(fit)\$coefficients[\"$pres[$i]\",1]\npval.$pres[$i] <- summary(fit)\$coefficients[\"$pres[$i]\",4]\n";
	}
	print RCODE "newdat <- data.frame(";
	for($i=1;$i<$#pres;$i++){
		$j=$i+2;
		print RCODE "$pres[$i]=dat[,$j],";
	}
	$j=$#pres+2;
	print RCODE "$pres[$#pres]=dat[,$j])\n";
	print RCODE "pred <- predict(fit, newdat)\n";
	print RCODE "pr <- data.frame(dat[,1],dat[,2],pred)\n";
	print RCODE "write.table(pr,file=\"predicted_vals\",append=T,quote=F,row.names=F,col.names=F)\n";
