...
print STDERR "Bed file : $ARGV[0]\nWig OUT : $ARGV[1]\nMIN: $ARGV[2]\nMAX: $ARGV[2]\n";

open(FILE,"/home/jsarthy/code/hg19_chr.sizes") || die "$!\n";
...
while(chomp($line=<FILE>)){
  ...
  if($#temp != 5){
    ...
    if(exists $chr_size{$temp[0]}){
    	$lower=int(($temp[1]/10) + 0.5);
    	$upper=int(($temp[2]/10) + 0.5);
    	for($i=$lower;$i<=$upper;$i++){
				if($i< ($chr_size{$temp[0]}/10)){
      		$read{$temp[0]}{$i}++;
      		$count++;
				}
   		}
